...
[![Build Status](https://secure.travis-ci.org/hiddentao/wiseplat-client-binaries.png?branch=master)](http://travis-ci.org/hiddentao/wiseplat-client-binaries) [![NPM module](https://badge.fury.io/js/wiseplat-client-binaries.png)](https://badge.fury.io/js/wiseplat-client-binaries) [![Twitter URL](https://img.shields.io/twitter/url/http/shields.io.svg?style=social&label=Follow&maxAge=2592000)](https://twitter.com/hiddentao)
...
```shell
npm install --save wiseplat-client-binaries
```
...
For example, a config object which specifies the [Gwsh client](https://github.com/wiseplat/go-wiseplat) for only 64-bit Linux platforms and the [Parity client](https://github.com/wiseplat/parity) for only 32-bit Windows platforms might be:
...
```js
const Manager = require('wiseplat-client-binaries').Manager;

// construct
const mgr =  new Manager(config);
```

**Note:** If no config is provided then the default config ([src/config.json](https://github.com/wiseplat/wiseplat-client-binaries/blob/master/src/config.json)) gets used.
